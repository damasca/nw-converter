...
#  Solarus conversion edit by Zaidyer
...
# untidy Solarus-specific tile encoding system here
    ...
    for y in range(64):
        for x in range(64):
            ...
            data.append('tile{\n' + 
                ...
                'width = 8,\n' + 
                'height = 8,\n' + 
                'pattern = "' + str(tiled_index) + '",\n' + 
                '}\n')
        
    return "\n".join(data)

def encode_the_links(level):
    data = []
    for link in level.links:
        data.append('teletransporter{\n' + 
            'layer = 0,\n' + 
            'x = ' + str(link.area[0] * 8) + ',\n' + 
            'y = ' + str(link.area[1] * 8) + ',\n' + 
            'width = ' + str(link.area[2] * 8) + ',\n' + 
            'height = ' + str(link.area[3] * 8) + ',\n' + 
            'destination_map = "' + str(link.target) + '",\n' + 
            '}\n')
...
def encode_as_tmx(level, tileset_path):
    ...
    data.text = encode_as_csv(level) + encode_the_links(level)
...
if __name__ == "__main__":
    ...
    out_path = os.path.join("out", level_name + ".dat")
